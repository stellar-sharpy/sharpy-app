...
var SharpyClient = class {
  ...
  async getInvoiceCount() {
    const account = await this.server.getAccount("GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF");
    const contract = new stellarSdk.Contract(this.config.contractId);
    const tx = new stellarSdk.TransactionBuilder(account, { fee: stellarSdk.BASE_FEE, networkPassphrase: this.config.networkPassphrase }).addOperation(contract.call("get_invoice_count")).setTimeout(30).build();
    const sim = await this.server.simulateTransaction(tx);
    if ("error" in sim) throw new Error(`Simulation failed: ${sim.error}`);
    return Number(stellarSdk.scValToNative(sim.result.retval) ?? 0);
  }
  /**
  ...
  async getInvoicesByPayer(payer) {
    const account = await this.server.getAccount("GAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAWHF");
    const contract = new stellarSdk.Contract(this.config.contractId);
    const tx = new stellarSdk.TransactionBuilder(account, { fee: stellarSdk.BASE_FEE, networkPassphrase: this.config.networkPassphrase }).addOperation(contract.call("get_invoices_by_payer", new stellarSdk.Address(payer).toScVal())).setTimeout(30).build();
    const sim = await this.server.simulateTransaction(tx);
    if ("error" in sim) throw new Error(`Simulation failed: ${sim.error}`);
    const raw = stellarSdk.scValToNative(sim.result.retval);
    return raw.map(Number);
  ...
  async completeCctpInbound(caller, message, attestation) {
    ...
    const args = [
      stellarSdk.nativeToScVal(messageBytes, { type: "bytes" }),
      stellarSdk.nativeToScVal(attestationBytes, { type: "bytes" })
    ];
...
function mapInvoice(raw) {
  ...
  return {
    ...
    claimed: raw.claimed?.map((c) => BigInt(c)) || [],
...
//# sourceMappingURL=chunk-EM3TOUYE.cjs.map
//# sourceMappingURL=chunk-EM3TOUYE.cjs.map